...
and returns the structured result. All UI copy and API error messages are in Portuguese.
...
`http://localhost:5000/api/ocr`, which is hardcoded as `OCR_ENDPOINT` in `app.js`.
...
`app.py` is the only HTTP surface (`POST /api/ocr`) and orchestrates two independent, swappable steps:
...
1. `ocr.py` — Google Cloud Vision `document_text_detection` (images) / `batch_annotate_files` (PDF,
   ...
   failure doesn't fail the request — it's recorded in the response's `notes` and that file is skipped.
